...
class FileReader(metaclass=ABCMeta):
    def __init__(self, filename, header):
        ...
        self.line_count = 0
        self.iteration_methods = {'lazy_read': self.__lazy_read, 'chunky_lazy_read': self.__chunky_lazy_read,
    ...
    def set_iteration_method(self, iteration_method):
        self.iteration_method = self.get_iteration_methods()[iteration_method]

        if iteration_method is 'chunky_lazy_read':
            self.__set_line_width_for_chunks()
            print(self.__line_width)

    def get_iteration_methods(self):
        return self.iteration_methods
    ...
    def __lazy_read(self):
        with open(self.filename, "rt") as f:
            reader = csv.DictReader(f, fieldnames=self.header, delimiter=',')
                ...
                if lines:
                    ...
                    for line in lines_list:
                        ...
                        if self.filter_expression:
                            if self.filter_expression in line:
                                yield line.split(',')
                            else:
                                continue
                        else:
